...
- [ ] CB-MONTHLY-REVIEW-BUYHOLD (ACTIVE, HIGH) — monthly_watchlist_review.py fires 2026-05-01 05:00 UTC with latent BUY_HOLD-blind mechanism: 'closed<30 AND days_active>60 → INACTIVE_CULL (remove from JSON)' at lines 73-76. Pure BUY_HOLD wallets always have closed_trades=0 (compute_profile can't pair-match buys-only), so any BUY_HOLD wallet in WATCHLIST_ONLY >60 days old will be physically removed. Third surface of the BUY_HOLD bug class identified today (first: reconstruct_pnl Apr 8/14, fixed; second: activity-freshness Apr 18, correct-by-design; third: closed_trades criterion in monthly review, ARMED). Must be resolved before May 1. Options: (a) fix monthly_watchlist_review.py to exempt BUY_HOLD wallets, (b) temporarily disable the monthly cron pending CB-HARVESTER-BUYHOLD delivery, (c) promote all Mode B BUY_HOLD restores straight to T2 (bypasses WATCHLIST_ONLY path). Gated on CB-HARVESTER-BUYHOLD for (a)/(b); decision for (c) can be made independently in Mode B.
...
- [ ] CB-HARVESTER-BUYHOLD (ACTIVE, HIGH) — Extend harvester to attribute P&L / band-rate to buy-only (BUY_HOLD) wallets. Unblocks CB-BUYHOLD-MONITOR and protects future culls from the blind spot. [2026-04-23 PM uplift to HIGH]: Now blocks THREE separate TODOs: (1) CB-BUYHOLD-MONITOR (Apr 8 residuals 0xdbdd45 + 0xeca1e9), (2) CB-APR14-AUDIT140-FINDING (if restore discussion resumes for the 139 BUY_HOLD-blind false-positives), (3) CB-MONTHLY-REVIEW-BUYHOLD (May 1 latent bug in monthly_watchlist_review.py). All three converge on the same missing capability: harvester attribution of P&L/band-rate to buy-only wallets via mark-to-market or sample-redemption. Three-TODO convergence is signal enough to elevate.
